...
export default function Test() {
  ...
  const [dropSpeed, setDropSpeed] = useState(100);
...
useEffect(() => {
  ...
  const drawKeypoints = (keypoints) => {
    ...
    keypoints.forEach(keypoint => {
        const originalX = keypoint.position.x;
        const originalY = keypoint.position.y;
        
        // Log the original coordinates
        console.log(`Original Coordinates: (${originalX}, ${originalY})`);
        
        // Apply scaling
        const scaledX = originalX * 0.8;
        const scaledY = originalY * 0.8;
    ...
    adjacentKeyPoints.forEach(points => {
        ctx.beginPath();
        ctx.moveTo(points[0].position.x * 0.8, points[0].position.y * 0.8);
        ctx.lineTo(points[1].position.x * 0.8, points[1].position.y * 0.8);
        ctx.lineWidth = 2;
  ...
  const userMessage = generateMessage(armAngle, dropSpeed, elbowAngle, curlSpeed);
    ...
    return (
      ...
      <Box flex="1" marginRight={5} display="flex" flexDirection="column" alignItems="flex-start" position="relative" width="640px" height="480px">
        <video ref={videoRef} width="640" height="480" playsInline style={{ objectFit: 'contain' }} />
